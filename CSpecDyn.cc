...
void CSpecDyn::time_step()
{
  ...
  { 
    ...
    exp_diff_V_0 = exp(- nu  * k2h[id] * dt);
    exp_diff_B_0 = exp(- eta * k2h[id] * dt);
  ...
  { 
    ...
    exp_diff_V_0 = exp(- 0.5 * nu  * k2h[id] * dt);
    exp_diff_B_0 = exp(- 0.5 * eta * k2h[id] * dt);
...
void CSpecDyn::dealias(CX* fieldX, CX* fieldY, CX* fieldZ)
{
  ...
  for(int iz = 0; iz<size_F[2]; iz++){
    ...
    if( fabs(kx[ix]) > kmax ||  fabs(ky[iy]) > kmax ||  fabs(kz[iz]) > kmax/Lz_L)
    {
      fieldX[id] = 0.;
      fieldY[id] = 0.;
      fieldZ[id] = 0.;
