...
out_interval(OUT_INTERVAL), end_simu(END_SIMU), setup(SETUP)
{
  ...
  normal_eng = std::mt19937(myRank+5);
  ...
  switch(setup)
  {
    ...
    case 1:
      ...
      for(int iz = 0; iz < size_R[2]; iz++){
        ...
        Vz_R[id] = cos(x_val)*cos(y_val)*cos(z_val);
...
void CSpecDyn::time_step()
{
  ...
  { 
    ...
    exp_diff_V_0 = 1.;// exp(- nu  * k2[id] * dt);
    exp_diff_B_0 = 1.;// exp(- eta * k2[id] * dt);
  ...
  { 
    ...
    exp_diff_V_0 = 1.;// exp(- 0.5 * nu  * k2[id] * dt);
    exp_diff_B_0 = 1.;// exp(- 0.5 * eta * k2[id] * dt);
    exp_diff_V_1 = 1.;// exp(+ 0.5 * nu  * k2[id] * dt);
    exp_diff_B_1 = 1.;// exp(+ 0.5 * eta * k2[id] * dt);
...
void CSpecDyn::Alvelius()
{
  ...
  for(int id = 0; id < size_F_tot; id++){
    ...
    energy_F_loc += (Fx*Fx+Fy*Fy+Fz*Fz);

  }
  
  double N3_inv = 1./(N*N*N);

  energy_F_loc *= 0.5*N3_inv; // mean energy
  energy_F_loc *= N3_inv; // Parseval
  MPI_Allreduce(&energy_F_loc, &energy_F, 1, MPI_DOUBLE, MPI_SUM, comm);
  
  double norm = sqrt( P/dt  / energy_F);
                        ...
                        double del_t)
{
  ...
  for(int iz = 0; iz<size_F[2]; iz++){
    ...
    RHSV_X[id] = PI2 * IM * kx[ix] * V_X[id];
    RHSV_Y[id] = PI2 * IM * ky[iy] * V_Y[id];
    RHSV_Z[id] = PI2 * IM * kz[iz] * V_Z[id];
